...
def save_result(result_load_path,label_pred,parameter1,parameter2):
    fp = open(result_load_path,'w')
    fp.write('predictional result of foothill model data,depth:50m \r\n')
    fp.write('Earthquake parameters：Real parameters（Predicted parameters）\r\n')   
    
    for i in range(label_pred.shape[0]):
        onelabel_pred = label_pred
        pos_pred = np.where(onelabel_pred == np.max(onelabel_pred))
        para1_pred = pos_pred[0][0]
        para2_pred = pos_pred[1][0]
        pos_pred_value = onelabel_pred[para1_pred][para2_pred]
        output  = '{0}: {1} \t'.format(parameter1,'%6.2f'%p1(para1_pred))
        output += '{0}: {1} \t'.format(parameter2,'%6.2f'%p2(para2_pred))
        output += '{0}\r\n'.format('%6.4f' % pos_pred_value)
        # print(output)
        fp.write(output)
    fp.close()    
...
def save_label_pred(result_load_path,data,label_pred,label_predpath):
    ...
    for j in index:
        fp=open(result_load_path+'time_save.txt','w')
        x = label_pred[j][:]
